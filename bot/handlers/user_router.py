...
async def cmd_start(message: Message) -> None:
    await message.reply(f'Добро пожаловать, {message.from_user.full_name}', reply_markup=kb.main_keyboard(message.from_user.id))
...
async def cmd_info(message: Message):
    ...
    """
    await message.answer(f"Описание бота```{bot_description}```", parse_mode='Markdown', reply_markup=kb.back_keyboard())


@user_router.callback_query(F.data == 'back_home')
async def go_back(query: CallbackQuery):
    await query.message.edit_text('Вы вернулись назад', reply_markup=kb.main_keyboard(query.from_user.id))
